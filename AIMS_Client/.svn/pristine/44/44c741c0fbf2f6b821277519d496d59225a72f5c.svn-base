...
//	}
	...
	@FXML
	void homeBtn(ActionEvent event) throws IOException, ClassNotFoundException {
		
		if(AlertUtil.showAlert((Stage) memberModi.getScene().getWindow(), "메인페이지로  ", "이동하시겠습니까?", "이동", "취소")) {
			FXMLLoader loader = new FXMLLoader(Class.forName("kr.or.ddit.controller.adminmypage.AdminMainController").getResource("../../fxml/adminmain/AdminMain.fxml"));
	...
	// 회원 수정 버튼을 누르면 화면 전환
	@FXML
	void memModiBtn(ActionEvent event) throws IOException, ClassNotFoundException {

		FXMLLoader loader = new FXMLLoader(
				Class.forName("kr.or.ddit.controller.adminmypage.AdminMainController").getResource("../../fxml/adminmypage/memberInfoModify.fxml"));
		Parent root = loader.load();
