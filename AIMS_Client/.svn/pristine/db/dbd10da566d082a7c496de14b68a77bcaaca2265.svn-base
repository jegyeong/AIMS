...
public class AgreeViewController {
    ...
    // 확인 버튼을 눌렀을 때
    @FXML
    void btnOk(ActionEvent event) throws ClassNotFoundException {
    	
    	try {
			// 확인 버튼을 누르면  회원가입 창으로 이동하는거  화면 전환
    		
    		
    		FXMLLoader loader = new FXMLLoader(Class.forName("kr.or.ddit.controller.AgreeView.AgreeViewController").getResource("../../fxml/member/Member.fxml"));
    		Parent root = loader.load();
